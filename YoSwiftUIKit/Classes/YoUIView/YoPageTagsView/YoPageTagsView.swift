...
//MARK: - 协议
public protocol YoPageTagsViewDelegate: AnyObject  {
    
    // 数据源 即有几个标题
    //- Parameter titleView: 对象
    func numberOfTitles(in titleView: YoPageTagsView) -> Array<String>
    
    /// 标题属性 可选 不设置取默认值
    /// - Parameter titleView: 对象
    func attributesOfTitles(for titleView: YoPageTagsView) -> YoTitleAttributes
    
    /// 点击标题事件
    func titleView(_ titleView: YoPageTagsView, didSelectedAt index: Int)
}

public extension YoPageTagsViewDelegate {
...
//MARK: - YoPageTagsView 类
public class YoPageTagsView: UIView {
    /// 代理
    public weak var delegate: YoPageTagsViewDelegate!
    ...
    public init(
        frame: CGRect,
        delegate: YoPageTagsViewDelegate,
        default index: Int = 0
    ) {
        super.init(frame: frame)
        self.delegate = delegate
        assert(index >= 0 && index <= titles.count - 1, "数组越界错误, 请检查下标大小值")
        currentIndex = index
        createSubviews()
    }
...
//MARK: - 布局|配置
private extension YoPageTagsView {
