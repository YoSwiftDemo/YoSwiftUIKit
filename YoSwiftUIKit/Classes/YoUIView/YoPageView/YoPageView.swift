...
class YoPageView: UIView {
    public weak var delegate: YoPageViewDelegate!   // 代理
    public var currentIndex: Int = 0         // 默认 Index
    private var beginDragging: Bool = false  //监控 是否开始拖拽
    private var beginDraggingOffsetX: CGFloat = 0 //纪录拖拽开始时的偏移量
    /// 页面数据
    ...
    private lazy var collectionView: UICollectionView = {
        let layout = UICollectionViewFlowLayout() //布局
        ...
        collectionView.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "YoPageViewCell")
...
extension YoPageView: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "YoPageViewCell", for: indexPath)
...
extension YoPageView: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        progressRefresh(in: self, scrollView: scrollView)
    }
...
private extension YoPageView {
    ...
    /// 滚动的百分比
    func progressRefresh(in pageView: YoPageView, scrollView: UIScrollView) {
